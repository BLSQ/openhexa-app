...
class DummyStorageClient(Storage):
    ...
    def delete_object(self, bucket_name: str, object_key: str):
        ...
        if (
            ...
            or object_key not in self._buckets[bucket_name]
        ):
            raise self.exceptions.NotFound(
                f"Object '{object_key}' not found in bucket '{bucket_name}'."
            )
        del self._buckets[bucket_name][object_key]
    ...
    def delete_bucket(self, bucket_name: str, force: bool = False):
        ...
        if force is False and self._buckets[bucket_name]:
    ...
    def generate_download_url(
        ...
        if (
            bucket_name not in self._buckets
            or target_key not in self._buckets[bucket_name]
        ):
            raise self.exceptions.NotFound(
    ...
    def _to_storage_object(self, bucket_name: str, object_key: str):
        if not self.bucket_exists(bucket_name):
            raise self.exceptions.NotFound(f"Bucket '{bucket_name}' not found.")
        if object_key not in self._buckets[bucket_name]:
            raise self.exceptions.NotFound(
                f"Object '{object_key}' not found in bucket '{bucket_name}'."
            )
        obj = self._buckets[bucket_name][object_key]
        if obj == {}:
    ...
    def list_bucket_objects(
        ...
        if bucket_name not in self._buckets:
            raise self.exceptions.NotFound(f"Bucket '{bucket_name}' not found.")
        object_keys = []
        for key in self._buckets[bucket_name].keys():
    ...
    def generate_upload_url(
        ...
        **kwargs,
        ...
        if bucket_name not in self._buckets:
            raise self.exceptions.NotFound(f"Bucket '{bucket_name}' not found.")
        if raise_if_exists and target_key in self._buckets[bucket_name]:
            raise self.exceptions.AlreadyExists(
