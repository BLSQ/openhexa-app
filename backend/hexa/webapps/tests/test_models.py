...
from hexa.webapps.models import GitWebapp, SupersetWebapp, Webapp, _parse_git_diff
...
class ParseGitDiffTest(TestCase):
    ...
    def test_added_file(self):
        ...
        files = _parse_git_diff(diff)
        self.assertEqual(len(files), 1)
        self.assertEqual(files[0]["filename"], "new.html")
    ...
    def test_deleted_file(self):
        ...
        files = _parse_git_diff(diff)
        self.assertEqual(len(files), 1)
        self.assertEqual(files[0]["filename"], "old.html")
    ...
    def test_multiple_files(self):
        ...
        self.assertEqual(files[0]["filename"], "index.html")
        self.assertEqual(files[0]["status"], "modified")
        self.assertEqual(files[1]["filename"], "style.css")
        self.assertEqual(files[1]["status"], "added")
        self.assertEqual(files[2]["filename"], "old.js")
        self.assertEqual(files[2]["status"], "deleted")

    def test_empty_diff(self):
        files = _parse_git_diff("")
        self.assertEqual(files, [])

    def test_renamed_file(self):
        diff = (
            "diff --git a/index.html b/iindex.html\n"
            "similarity index 100%\n"
            "rename from index.html\n"
            "rename to iindex.html\n"
        )
        files = _parse_git_diff(diff)
        self.assertEqual(len(files), 1)
        self.assertEqual(files[0]["filename"], "iindex.html")
        self.assertEqual(files[0]["previous_filename"], "index.html")
        self.assertEqual(files[0]["status"], "renamed")
        self.assertEqual(files[0]["additions"], 0)
        self.assertEqual(files[0]["deletions"], 0)

    def test_additions_deletions_count_excludes_header_lines(self):
        diff = (
