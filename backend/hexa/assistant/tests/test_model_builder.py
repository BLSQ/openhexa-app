...
class GetApiNameTest(SimpleTestCase):
    ...
    def test_vertex_uses_bare_model_ids(self):
        self.assertEqual(
            get_api_name(AiSettings.Provider.ANTHROPIC_VERTEX, AiSettings.Model.HAIKU),
            "claude-haiku-4-5",
        )
        self.assertEqual(
            get_api_name(AiSettings.Provider.ANTHROPIC_VERTEX, AiSettings.Model.SONNET),
            "claude-sonnet-4-6",
        )
        self.assertEqual(
            get_api_name(AiSettings.Provider.ANTHROPIC_VERTEX, AiSettings.Model.OPUS),
            "claude-opus-4-6",
        )
...
class AiModelBuilderTest(TestCase):
    ...
    def test_build_returns_built_model_for_vertex(self, mock_vertex_client):
        builder = AiModelBuilder(
            _make_ai_settings(
                AiSettings.Provider.ANTHROPIC_VERTEX,
        ...
        result = builder.build()
        self.assertIsInstance(result, BuiltModel)
        self.assertEqual(result.api_name, "claude-sonnet-4-6")
        self.assertEqual(result.provider_id, AiSettings.Provider.ANTHROPIC_VERTEX)
    ...
    def test_build_vertex_without_project_raises(self):
        builder = AiModelBuilder(
            _make_ai_settings(
                AiSettings.Provider.ANTHROPIC_VERTEX,
