...
from ._helpers import _make_tool_call_model, _make_zipfile, make_models, run_agent
...
class EditPipelineAgentExtraInstructionsTest(AgentTestCase):
    def _make_agent(self, pipeline=None):
        ...
        conversation.save()
        return EditPipelineAgent(conversation, make_models(TestModel()))
...
class EditPipelineAgentToolCallTest(AgentTestCase):
    ...
    def test_propose_pipeline_version_call_is_persisted(self):
        ...
        conversation.save()
        agent = EditPipelineAgent(conversation, make_models(model))
    ...
    def test_new_proposal_supersedes_previous_pending_one(self):
        ...
        run_agent(
            EditPipelineAgent(conversation, make_models(first_model)),
            "First change",
        )
        ...
        run_agent(
            EditPipelineAgent(conversation, make_models(second_model)),
            "Second change",
        )
