...
class BaseAgentRunTest(AgentTestCase):
    ...
    def test_run_saves_user_message(self):
        agent = BaseAgent(
            self.conversation, make_models(TestModel(custom_output_text="Hello!"))
        )
    ...
    def test_run_saves_assistant_message(self):
        ...
        agent = BaseAgent(
            self.conversation,
            make_models(TestModel(custom_output_text="Hello!", call_tools=[])),
        )
        run_agent(agent, "What can you do?")
    ...
    def test_run_updates_messages_history(self):
        agent = BaseAgent(
            self.conversation, make_models(TestModel(custom_output_text="Hi"))
        )
    ...
    def test_second_run_appends_to_history(self):
        agent = BaseAgent(
            self.conversation, make_models(TestModel(custom_output_text="Reply"))
        )
...
class BaseAgentToolCallTest(AgentTestCase):
    ...
    def test_tool_call_creates_tool_invocation_record(self):
        ...
        agent = _AgentWithFakeTool(self.conversation, make_models(model))
    ...
    def test_tool_call_with_error_response_sets_success_false(self):
        ...
        agent = _AgentWithFailingTool(self.conversation, make_models(model))
...
class ConversationNamingTest(AgentTestCase):
    ...
    def test_six_word_title_is_accepted_without_retry(self):
        title = "Tableau de bord lecture dynamique CSV"
        agent = BaseAgent(self.conversation, make_models(_make_naming_model(title)))
    ...
    def test_naming_usage_is_priced_with_the_utility_model(self):
        ...
        agent = BaseAgent(
            self.conversation,
            AgentModels(
                main=make_built_model(naming_model, api_name="main-model"),
                utility=make_built_model(naming_model, api_name="utility-model"),
            ),
        )
    ...
    def test_empty_title_retries_and_uses_second_attempt(self):
        agent = BaseAgent(
            self.conversation,
            make_models(_make_naming_model("", "Tableau de bord alertes")),
        )
        run_agent(agent, "Améliore ce tableau de bord")
        self.conversation.refresh_from_db()
    ...
    def test_empty_titles_fall_back_to_user_input_never_empty_name(self):
        agent = BaseAgent(self.conversation, make_models(_make_naming_model("   ")))
        run_agent(agent, "Améliore ce tableau de bord pour que les données soient lues")
        self.conversation.refresh_from_db()
        self.assertEqual(
