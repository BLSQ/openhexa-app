...
class GetHelpOrDocTest(MCPTestCase):
    ...
    def test_docstring_lists_curated_topics(self):
        expected = (
            ...
            "    - sql-widgets: Data Studio draws a chart instead of a table when a query returns a specific set of column names: aliasing columns to `bar_label` and `bar_quantity` renders a bar chart, turning a saved query into a small dashboard.\n"
            ...
            "    - toolbox-hexa: The OpenHEXA SDK ships a typed `OpenHexaClient` to interact programmatically with the OpenHEXA platform — workspaces, pipelines, runs, datasets and webapps — with full type hints. It replaces the legacy `openhexa.toolbox.hexa` client.\n"
