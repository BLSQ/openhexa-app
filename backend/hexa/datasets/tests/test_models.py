...
class DatasetLinkPipelineRunUserTest(BaseTestMixin, TestCase):
    ...
    def test_pipeline_user_sees_org_shared_datasets_via_dataset_queryset(self):
        """DatasetQuerySet.filter_for_user must mirror the DatasetLinkQuerySet
        behavior for service principals: shared-with-organization datasets in
        the same org are visible. This pins the fix for the prior asymmetry
        where Dataset.filter_for_user short-circuited service principals to
        only see datasets linked into their own workspace.
        """
        self.DATASET_IN_OTHER_WORKSPACE_SAME_ORG.shared_with_organization = True
        self.DATASET_IN_OTHER_WORKSPACE_SAME_ORG.save()
