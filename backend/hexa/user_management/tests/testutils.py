...
def create_organization(name: str = "Test Organization", **overrides) -> Organization:
    """Create an Organization with sensible defaults for tests.

    New required fields only need a default added here rather than in every test
    that builds an organization. The slug is generated by ``Organization.save()``.
    """
    defaults = {"organization_type": "CORPORATE"}
    return Organization.objects.create(name=name, **{**defaults, **overrides})
