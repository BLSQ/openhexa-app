...
def build_page_request(
    ...
    *,
    ...
    A call with no ``orderBy``, ``page`` or ``after`` runs unwrapped, exactly as
    before this argument set existed. ``orderBy`` alone is the first page of both
    modes.
    """
    order_by = order_by or []
    if page is None and after is None and not order_by:
        return None
    ...
    if page is not None and after is not None:
        raise InvalidPagination("page and after cannot be combined.")
    if after is not None:
        ...
        try:
            keyset = cursor_codec.decode_cursor(after, prepared.body, order_by)
        except cursor_codec.InvalidCursor as e:
            raise InvalidCursor(str(e))
        return CursorPage(order_by=order_by, per_page=per_page, keyset=keyset)
