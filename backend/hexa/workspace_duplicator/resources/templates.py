"""Pipeline template migration: server-wide templates copied to target.

Templates are globally visible, so this runs once per target server,
independently of any workspace migration (it is not part of the workspace copier
registry — the #2 flow invokes ``migrate_all`` directly). Each source template is
recreated on the target by:

1. Locating a host pipeline that can back the template on target:
   - Prefer the *already-migrated* source pipeline (in its original
     workspace on target) if it exists — keeps the underlying pipeline
     visible inside its real workspace.
   - Otherwise create a host pipeline inside a dedicated
     "Template pipelines" workspace.
2. Ensuring each template version's underlying pipeline version exists on
   the host pipeline (upload the source zipfile if missing).
3. Calling ``createPipelineTemplateVersion`` per missing template version,
   in source ``versionNumber`` order. The first call also creates the
...
Idempotent: matches templates by name (globally unique) and existing
...
def _ensure_host_workspace(target: Client) -> str:
    """Return the slug of an existing target workspace named HOST_WORKSPACE_NAME, creating it if needed."""
    page = 1
    while True:
    ...
    created = target.create_workspace(
        input=CreateWorkspaceInput(
            ...
            description=(
                "Host workspace for source pipelines backing migrated templates. "
                "Auto-created by migrate_templates.py."
            ),
            countries=[],
...
def _resolve_or_create_host_pipeline(
    ...
    Reuses the source pipeline in its original workspace on target if it
    exists there (because the workspace migration already brought it
    over). Otherwise reuses or creates a host pipeline in
    HOST_WORKSPACE_NAME, matched by source pipeline name (since the
    server re-derives code from name on create, e.g. dropping
    underscores).
    """
...
def _ensure_host_pipeline_version(
    ...
    versionNumber exists; otherwise uploads the source zipfile and
    records the new id in `host_versions_by_number`.
    """
