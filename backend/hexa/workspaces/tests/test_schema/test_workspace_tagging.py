...
WORKSPACES_QUERY = """
    query workspaces($organizationId: UUID, $tags: [String!], $query: String) {
        workspaces(organizationId: $organizationId, tags: $tags, query: $query) {
            totalItems
            items {
                slug
            }
...
class WorkspaceTaggingTest(GraphQLTestCase):
    ...
    def test_search_matches_tag_name(self):
        self.client.force_login(self.USER_ADMIN)
        self.set_tags(self.WORKSPACE_2.slug, ["malaria"])

        result = self.run_query(
            WORKSPACES_QUERY,
            {"organizationId": str(self.ORGANIZATION.id), "query": "malaria"},
        )["data"]["workspaces"]

        self.assertEqual(1, result["totalItems"])
        self.assertEqual([{"slug": self.WORKSPACE_2.slug}], result["items"])

    def test_search_matches_a_partial_tag_name(self):
        """Tags match on substrings, like the workspace name does."""
        self.client.force_login(self.USER_ADMIN)
        self.set_tags(self.WORKSPACE_2.slug, ["firsttag"])

        result = self.run_query(
            WORKSPACES_QUERY,
            {"organizationId": str(self.ORGANIZATION.id), "query": "first"},
        )["data"]["workspaces"]

        self.assertEqual(1, result["totalItems"])
        self.assertEqual([{"slug": self.WORKSPACE_2.slug}], result["items"])

    def test_search_matches_name_or_tag_without_duplicates(self):
        """A workspace matching on both its name and one of its tags is returned once."""
        self.client.force_login(self.USER_ADMIN)
        self.set_tags(self.WORKSPACE.slug, ["senegal"])
        self.set_tags(self.WORKSPACE_2.slug, ["senegal", "malaria"])

        result = self.run_query(
            WORKSPACES_QUERY,
            {"organizationId": str(self.ORGANIZATION.id), "query": "senegal"},
        )["data"]["workspaces"]

        self.assertEqual(2, result["totalItems"])
        self.assertEqual(
            [{"slug": self.WORKSPACE_2.slug}, {"slug": self.WORKSPACE.slug}],
            result["items"],
        )

    def test_search_is_combined_with_the_tag_filter(self):
        self.client.force_login(self.USER_ADMIN)
        self.set_tags(self.WORKSPACE.slug, ["malaria"])
        self.set_tags(self.WORKSPACE_2.slug, ["malaria"])
