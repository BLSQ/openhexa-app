...
def accept_pending_invitations(apps, schema_editor):
    ...
    for invitation in invitations:
        ...
        # Add in attrs from custom `save` on the model (they're not ran in migrations)
        WorkspaceMembership.objects.create(
            workspace=invitation.workspace,
            user=existing_user,
            role=invitation.role,
            notebooks_server_hash=hashlib.blake2s(
                f"{invitation.workspace_id}_{existing_user.id}".encode(), digest_size=16
            ).hexdigest(),
