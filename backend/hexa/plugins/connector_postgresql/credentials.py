...
def get_env(
    ...
    if len(databases) > 0:
        ...
        for database, label in databases:
            ...
            # Build database URL with optional application_name for pipeline runs
            db_url = database.url
...
def pipelines_credentials(credentials: PipelinesCredentials):
    ...
    env, files = get_env(databases, run_id=credentials.run_id, pipeline_name=credentials.pipeline_name)
