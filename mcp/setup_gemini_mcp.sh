#!/bin/bash
# =============================================================================
# OpenHEXA Gemini MCP Setup Script
# =============================================================================
# This script configures Gemini CLI with OpenHEXA MCP servers in a JupyterHub
...
#
# Usage: source setup_gemini_mcp.sh
#        or: ./setup_gemini_mcp.sh
#
# Prerequisites:
# - Running inside a JupyterHub notebook with OpenHEXA workspace
# - Node.js/npm available (for Gemini CLI)
...
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}  OpenHEXA Gemini MCP Setup${NC}"
echo -e "${BLUE}============================================${NC}"
echo ""
...
if [ ! -f "$MCP_INSTALL_DIR/GEMINI.md" ] && [ ! -f "$MCP_INSTALL_DIR/CLAUDE.md" ]; then
    echo -e "${YELLOW}Warning: No instruction files (GEMINI.md or CLAUDE.md) found in repository${NC}"
fi
...
# Export for use in instruction files
export DASHBOARDS_DIR

echo ""

# =============================================================================
# Step 4: Check/Install Gemini CLI
# =============================================================================
echo -e "${YELLOW}[4/8] Checking Gemini CLI installation...${NC}"
...
# Check Node.js version (Gemini CLI requires v18+)
...
if command -v gemini &> /dev/null; then
    ...
    echo "  Current Gemini CLI version: $GEMINI_VERSION"
    ...
    if [ "$NEW_VERSION" != "$GEMINI_VERSION" ]; then
        echo -e "${GREEN}✓ Updated Gemini CLI: $GEMINI_VERSION -> $NEW_VERSION${NC}"
    else
        echo -e "${GREEN}✓ Gemini CLI is up to date: $GEMINI_VERSION${NC}"
    fi
else
    echo "Gemini CLI not found. Installing latest version..."

    # Install Gemini CLI globally (to user's npm-global directory)
    ...
    if command -v gemini &> /dev/null; then
        ...
        echo -e "${RED}Error: Gemini CLI installation failed${NC}"
        echo "Try running: source ~/.bashrc && gemini --version"
        exit 1
    fi
fi
...
# Auto-generated by setup_gemini_mcp.sh on $(date)
...
# Workspace Information (used by instruction files for dashboard building)
...
# =============================================================================
# Step 6: Ensure Python dependencies are installed
# =============================================================================
...
# =============================================================================
# Step 7: Configure Gemini settings with instruction files
# =============================================================================
echo -e "${YELLOW}[7/8] Configuring Gemini settings...${NC}"

# Create Gemini config directory
GEMINI_CONFIG_DIR="$HOME/.gemini"
mkdir -p "$GEMINI_CONFIG_DIR"

# Try to create project-level .gemini in current directory
# Fall back to home directory if permission denied
WORKSPACE_GEMINI_DIR="$(pwd)/.gemini"
if mkdir -p "$WORKSPACE_GEMINI_DIR" 2>/dev/null; then
    echo "  Using project-level config: $WORKSPACE_GEMINI_DIR"
else
    echo -e "${YELLOW}  Cannot create .gemini in current directory (permission denied)${NC}"
    echo "  Falling back to home directory..."
    WORKSPACE_GEMINI_DIR="$HOME/.gemini"
...
substitute_variables() {
    ...
    sed -e "s|\${BROWSER_API_URL}|$BROWSER_API_URL|g" \
        ...
        -e "s|\${GEMINI_CONFIG_DIR}|$GEMINI_CONFIG_DIR|g" \
        -e "s|\${CLAUDE_CONFIG_DIR}|$GEMINI_CONFIG_DIR|g" \
        "$src_file" > "$dest_file"
}

# Copy and substitute CLAUDE.md from the repository (renamed to GEMINI.md)
CLAUDE_MD_SOURCE="$MCP_INSTALL_DIR/CLAUDE.md"

echo "  Processing CLAUDE.md -> GEMINI.md (renamed for Gemini)..."

if [ -f "$CLAUDE_MD_SOURCE" ]; then
    # Substitute variables and copy to workspace .gemini directory as GEMINI.md
    if substitute_variables "$CLAUDE_MD_SOURCE" "$WORKSPACE_GEMINI_DIR/GEMINI.md"; then
        # Also copy to global Gemini config directory so Gemini can find it
        substitute_variables "$CLAUDE_MD_SOURCE" "$GEMINI_CONFIG_DIR/GEMINI.md"
        echo -e "    ${GREEN}✓ GEMINI.md${NC}"
    else
        echo -e "    ${RED}✗ Failed to process GEMINI.md${NC}"
        exit 1
    fi
else
...
echo ""
echo -e "${GREEN}✓ GEMINI.md processed successfully${NC}"
...
if [ -d "$SKILLS_SOURCE_DIR" ]; then
    ...
    for skill_dir in "$SKILLS_SOURCE_DIR"/*/; do
        ...
        mkdir -p "$WORKSPACE_GEMINI_DIR/skills/$skill_name"
        mkdir -p "$GEMINI_CONFIG_DIR/skills/$skill_name"

        # Copy entire skill directory structure
        cp -r "$skill_dir"* "$WORKSPACE_GEMINI_DIR/skills/$skill_name/" 2>/dev/null
        cp -r "$skill_dir"* "$GEMINI_CONFIG_DIR/skills/$skill_name/" 2>/dev/null

        # Find and substitute variables in all .md files within the skill directory
        find "$WORKSPACE_GEMINI_DIR/skills/$skill_name" -name "*.md" -type f | while read -r md_file; do
        ...
        find "$GEMINI_CONFIG_DIR/skills/$skill_name" -name "*.md" -type f | while read -r md_file; do
            sed -i \
                -e "s|\${BROWSER_API_URL}|$BROWSER_API_URL|g" \
                -e "s|\${HEXA_WORKSPACE}|$HEXA_WORKSPACE|g" \
                -e "s|\${WORKSPACE_DATABASE_DB_NAME}|$WORKSPACE_DATABASE_DB_NAME|g" \
    ...
    if [ $SKILLS_COPIED -gt 0 ]; then
        echo -e "${GREEN}✓ Processed $SKILLS_COPIED skills to .gemini/skills/${NC}"
    else
...
# =============================================================================
# Step 8: Configure MCP servers in Gemini settings.json
# =============================================================================
echo -e "${YELLOW}[8/8] Configuring MCP servers...${NC}"

# Gemini CLI uses settings.json for MCP configuration
...
# Using jq if available, otherwise use Python for JSON manipulation
if command -v jq &> /dev/null; then
    ...
    if [ -f "$GEMINI_SETTINGS_FILE" ]; then
        EXISTING_SETTINGS=$(cat "$GEMINI_SETTINGS_FILE")
    else
        EXISTING_SETTINGS="{}"
    fi
    ...
    MCP_CONFIG=$(cat <<EOF
{
  "openhexa": {
    "command": "python",
    "args": ["$MCP_INSTALL_DIR/openhexa_mcp_server.py"],
    "env": {
      "HEXA_SERVER_URL": "$HEXA_SERVER_URL",
      "HEXA_TOKEN": "$HEXA_TOKEN"
    },
...
EOF
    ...
    # Add PostgreSQL server if available
    if [ "$HAS_DB" = true ] && [ -n "$POSTGRES_CONN_STRING" ]; then
        MCP_CONFIG=$(echo "$MCP_CONFIG" | jq --arg conn "$POSTGRES_CONN_STRING" --arg path "$MCP_INSTALL_DIR/postgres_mcp_server.py" '. + {
          "postgres": {
            "command": "python",
            "args": [$path],
            "env": {
    ...
    FINAL_SETTINGS=$(echo "$EXISTING_SETTINGS" | jq --argjson mcp "$MCP_CONFIG" '.mcpServers = $mcp')

    echo "$FINAL_SETTINGS" > "$GEMINI_SETTINGS_FILE"
else
    # Use Python for JSON manipulation (always available in JupyterHub)
    echo "  Using Python for configuration..."

    python3 << PYEOF
import json
import os

settings_file = "$GEMINI_SETTINGS_FILE"
...
mcp_servers = {
    "openhexa": {
        "command": "python",
        "args": [f"{mcp_install_dir}/openhexa_mcp_server.py"],
        "env": {
            "HEXA_SERVER_URL": hexa_server_url,
            "HEXA_TOKEN": hexa_token
        },
...
# Add PostgreSQL server if available
if has_db and postgres_conn:
    mcp_servers["postgres"] = {
        "command": "python",
        "args": [f"{mcp_install_dir}/postgres_mcp_server.py"],
        "env": {
...
echo -e "${GREEN}✓ MCP servers configured in $GEMINI_SETTINGS_FILE${NC}"
...
echo "  - $GEMINI_SETTINGS_FILE (MCP server config)"
echo "  - $WORKSPACE_GEMINI_DIR/*.md ($MD_FILES_COPIED guideline files)"
echo "  - $GEMINI_CONFIG_DIR/*.md (global copy of guidelines)"
echo ""
...
echo -e "${GREEN}You can run 'gemini' from any directory - MCP servers are registered globally.${NC}"
echo ""
echo "To start using Gemini with MCP servers:"
echo "  1. Run: gemini"
echo "  2. The instruction files are available in ~/.gemini/"
echo "  3. Ask Gemini to explore your workspace or query your database"
echo ""
...
echo "  gemini --version    - Check Gemini CLI version"
echo ""
echo -e "${YELLOW}Note: Make sure you have authenticated with Google:${NC}"
echo "  The free tier with a personal Google Account gives you access to"
echo "  Gemini 2.5 Pro with 1M token context window (60 req/min, 1000 req/day)"
echo ""
echo "  Alternatively, set a Gemini API key:"
echo "  export GEMINI_API_KEY=your-api-key"
echo ""
echo -e "${BLUE}Troubleshooting:${NC}"
echo "  - Check settings: cat ~/.gemini/settings.json"
echo "  - Run 'gemini --version' to check your Gemini CLI version"
echo "  - Ensure you're running the latest: npm update -g @google/gemini-cli"
echo ""
