...
def on_pydb_init(py_db):
    if PydevdCustomization.DEBUG_MODE == "debugpy-dap":
        ...
        pydev_log.debug(
            "WARNING: unknown debug mode: %s", PydevdCustomization.DEBUG_MODE
        )

    if PydevdCustomization.PREIMPORT:
        pydev_log.debug("Preimport: %s", PydevdCustomization.PREIMPORT)
        try:
            sys_path_entry, module_name = PydevdCustomization.PREIMPORT.rsplit(
                ";", maxsplit=1
            )
        except Exception:
            ...
            try:
                ...
                pydev_log.exception(
                    "Error importing %s (with sys.path entry: %s)"
                    % (module_name, sys_path_entry)
                )
