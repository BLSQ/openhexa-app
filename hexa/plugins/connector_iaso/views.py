...
def datasource_detail(request: HttpRequest, datasource_id: uuid.UUID) -> HttpResponse:
    ...
    orgunit_grid = OrgUnitGrid(
        iaso_account.iasoorgunit_set.prefetch_indexes().select_related("iaso_account"),
        parent_model=iaso_account,
        per_page=5,
        paginate=False,
        more_url=reverse(
...
def form_detail(
    ...
    breadcrumbs = [
        ...
        (_("Forms"), "connector_iaso:form_index", iasoaccount_id),
    ]
...
def iasoorgunit_detail(
    ...
    orgunit_object = get_object_or_404(
        iaso_account.iasoorgunit_set.prefetch_indexes(), iaso_id=iaso_id
    )
    ...
    breadcrumbs = [
        (_("Catalog"), "catalog:index"),
        (iaso_account.name, "connector_iaso:datasource_detail", iasoaccount_id),
        (_("OrgUnit"), "connector_iaso:orgunit_index", iasoaccount_id),
    ]
