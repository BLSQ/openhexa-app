...
class IASOAccount(Datasource):
    ...
    def sync(self):
        iasofroms = get_forms_json(iaso_account=self)
        iasoorgunits = get_orgunits_level12(iaso_account=self)
        ...
        with transaction.atomic():
            ...
            with transaction.atomic():
                ...
                # AISO OrgUnits
                remote = set()
                local = {x.iaso_id: x for x in self.iasoorgunit_set.all()}

                for orgunit in iasoorgunits:
                    key = orgunit["id"]
                    remote.add(key)
                    if key in local:
                        if datetime.fromtimestamp(form.get("updated_at")).replace(
                        ...
                        IASOOrgUnit.create_from_json(self, orgunit)
                        created_count += 1
    ...
    def content_summary(self):
        count = self.iasoform_set.count() + self.iasoorgunit_set.count()
...
class IASOApiToken(Base):
    ...
    iaso_account = models.ForeignKey(
        "IASOAccount", null=False, on_delete=models.CASCADE
    )
