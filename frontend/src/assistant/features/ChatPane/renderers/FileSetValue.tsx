...
export default function FileSetValue({ files }: { files: FileEntry[] }) {
  const { t } = useTranslation();
  // A single proposed file is almost always the thing to read, so open it up front.
  const [open, setOpen] = useState<Record<number, boolean>>(
    files.length === 1 ? { 0: true } : {},
  );
        ...
        return (
          <div key={i}>
            ...
            >
              ...
              <span className="truncate font-mono text-gray-700">{name}</span>
